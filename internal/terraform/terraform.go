...
func GetTerraformVersion() (*common.Version, error) {
	...
	if (majorVersion == 0 && minorVersion < 11 || minorVersion > 15) || (majorVersion == 1 && minorVersion > 4) {
		return nil, fmt.Errorf("terratag only supports Terraform from version 0.11.x and up to 1.4.x - your version says %s", outputAsString)
	}
